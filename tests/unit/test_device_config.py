...
class TestGetDevice:
    ...
    def test_returns_cpu_when_set_to_cpu(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Return 'cpu' when TUBE_SCOUT_DEVICE=cpu."""
        monkeypatch.setenv("TUBE_SCOUT_DEVICE", "cpu")
        assert get_device() == "cpu"

    def test_raises_for_invalid_value(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_raises_for_empty_string(self, monkeypatch: pytest.MonkeyPatch) -> None:
