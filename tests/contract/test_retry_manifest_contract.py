...
schema_version=1, alias, updated_at (ISO 8601 tz-aware), entries[].
...
# T023-1: top-level 필드 schema_version == 1
def test_contract_schema_version_is_1(tmp_path: Path) -> None:
    ...
    assert data["schema_version"] == 1, "schema_version must be integer 1"
...
def test_contract_entry_field_set_complete(tmp_path: Path) -> None:
    ...
    row = data["entries"][0]
    required = {"video_id", "title", "failed_stage", "failure_reason", "last_attempt_at", "attempt_count"}
    assert required == set(row.keys()), f"Entry fields mismatch: {set(row.keys())}"


# T023-6: failed_stage Literal 제약 ('transcript' | 'fingerprint')
def test_contract_entry_failed_stage_is_literal(tmp_path: Path) -> None:
    ...
    data = json.loads(path.read_text(encoding="utf-8"))
    row = data["entries"][0]
    assert row["failed_stage"] in ("transcript", "fingerprint"), (
        f"failed_stage must be 'transcript' or 'fingerprint', got {row['failed_stage']!r}"
    )
...
# T023-9: video_id non-empty string
def test_contract_entry_video_id_nonempty(tmp_path: Path) -> None:
    ...
    data = json.loads(path.read_text(encoding="utf-8"))
    row = data["entries"][0]
    assert isinstance(row["video_id"], str) and len(row["video_id"]) > 0
