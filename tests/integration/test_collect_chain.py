...
class TestCollectVideosThenTranscripts:
    ...
    def test_transcripts_uses_videos_project(self, tmp_path: Path) -> None:
        ...
        with (
            patch(
                "tube_scout.cli.collect.YouTubeDataService", return_value=videos_service
            ),
            patch(
        ...
        assert latest_link.is_symlink(), (
            "projects/latest must exist after collect videos"
        )
        ...
        with (
            patch(
                ...
                return_value=transcript_service,
            ),
        ):
    ...
    def test_no_empty_sibling_projects_after_chain(self, tmp_path: Path) -> None:
        ...
        for proj in real_projects:
            ...
            assert has_file, (
                f"Empty sibling project found: {proj} (01_collect/ is empty or missing)"
            )
