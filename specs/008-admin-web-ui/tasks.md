...
- [ ] T035 Implement `src/tube_scout/web/jobs/pipeline.py` — 7-stage orchestrator that calls existing services in order: `services.youtube_data.list_videos` → `services.youtube_data.fetch_metadata` → `services.transcript.collect` → `services.youtube_analytics.fetch_retention` → `services.youtube_analytics.fetch_analytics` → spec 007 `services.reuse_detection.scan` → `services.video_filter_service.generate_reports`. Each stage updates progress + checkpoints. Depends on T034.
...
- [ ] T035-bis [US1] Refactor `src/tube_scout/cli/collect.py` to extract a Typer-free internal helper `_collect_all_for_web(department_alias, professor, course, period_start, period_end, project_dir)`, then update `src/tube_scout/web/jobs/pipeline.py` to call it from listing/metadata/transcripts/retention/analytics stages. Reporting stage calls `tube_scout.reporting/...` (e.g. `bundle_report.BundleReportGenerator.generate(...)` to produce the v1v3 HTML/PDF/Excel + reuse HTML/Excel artifacts). Reuse-detection stage calls spec 007 module if present, else skips with WARN log. Replace `pipeline.not_integrated` raises with the integrated calls. RED test: `tests/integration/test_pipeline_real_services.py` covering happy-path + spec-007-missing graceful skip + OAuth expired + quotaExceeded + empty-list paths. Removes the T035 stub note and the `pipeline.not_integrated` error code.
