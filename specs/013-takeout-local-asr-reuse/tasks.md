...
- [ ] T097 [P] Update `CLAUDE.md` Recent Changes section with final spec 013 release notes (v0.5.0 release line, all 9 CLI commands listed, v4 migration summary).
...
- [ ] T101 Add 30-day follow-up reminder to `_workspace/measurement/30day_followup_plan.md` — Phase 3 first GA + 30 days → aggregate suspicion score weight commit (C-3 deferred). Document trigger: when ≥ 100 pairs accumulate `review_status='CONFIRMED_DUPLICATE'` or `'FALSE_POSITIVE'`, run weight derivation analysis (logistic regression / ROC).
...
- [ ] T104 Update version: `pyproject.toml` `version = "0.4.0"` → `"0.5.0"`. One dedicated commit per Constitution VIII Conventional Commits convention: `chore(release): bump version to v0.5.0 for spec 013 GA`.
...
- 측정 산출(T034, T041, T057, T058, T070)은 spec follow-up amendment 입력. 30일 후 가중치 공식 commit은 T101 follow-up reminder로 분리.
